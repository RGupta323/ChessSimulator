...
#only move one.
    ...
    def move(self,x):
        if(self.beginning==True): 
            if(x[0] in range(0,8) and canMove(board,self,x) and
               x[1] in range(8)): 
                self.position=x; 
            else: 
                return "Illegal Move" 
...
that pawn has no moves. But because of that funciton will be taken upon in another function'''
    ...
    if(obj.identify()=="Knight"): 
        ...
        if(obj.position[0]-2 in range(len(board)) and obj.position[1]+1 in
           range(len(board))):
            if(canMove(board,obj,[obj.position[0]-2,obj.position[1]+1])):
                if(checkOptions(obj,[obj.position[0]-2,obj.position[1]+1])
                   ):
...
def canMove(board, obj, position):
    if(board[position[0]][position[1]]=="X" and 0<=position[0]<len(board)
       and 0<=position[1]<len(board)):
        return True
    else:
        return False
...
######################################################################
#function to 'fix' the board, keep getting bugs where King 1 that keeps
#appearing multiple times throughout the board.
#To fix that, this function fixBoard() rewrite the board to make sure all
#pieces are in the actual positions
def fixBoard(board,bp,wp):
    pieces=bp+wp
    grid=createBoard() #creating a new board to put the pieces with
    for element in pieces:
        grid[element.position[0]][element.position[1]]=element.ID()
    return grid 
#######################################################################
#Testing
board=createBoard()
...
def play():
    ...
    options=createDict(bp,wp,board)
    #need to account for capturing!!!! 
    while(keepGoing):
        options=createDict(bp,wp,board) #need this inside the loop to keep being updated
    #white turn, pick a piece that can play, get the options grid, and choose a position and play... 
        print(options)
        if(turn==0 or turn%2==0):
            print("White's Turn!")
            #okay so here your job is loop through until you find a
            #'playable' piece so to speak ie something that can actually move.
            #if a piece turns out to not be playable then you pick a new piece
            Notplayable=True
            while(Notplayable):
                ...
                if(options[piece.identify()+" "+str(piece.num)]==[]):
                    while(options[piece.identify()+" "+str(piece.num)]==[]):
                        piece=wp[random.randint(0,len(wp)-1)]
                        previous=piece.position
                        #position=options[piece.identify()+" "+str(piece.num)][
                        #random.randint(0,len(options[piece.identify()+" "+str(piece.num)
                        #                            ])-1)]
                else:
                    position=options[piece.identify()+" "+str(piece.num)][
                        random.randint(0,len([piece.identify()+" "+str(piece.num)])
                                       -1)]
                    #print(position)
                if(options[piece.identify()+" "+str(piece.num)]!=[]):
                    position=options[piece.identify()+" "+str(piece.num)][
            ...
            if(canCapture(options,piece)!=False):
                ...
                piec=[element[1] for element in canCapture(options,piece)]
                if(position in positions):
                    #meaning if the position is going to capture the piece...
                    board[position[0]][position[1]]=piece.identify()+" "+str(piece.num)
                    print("{} captured {}!".format(piece.identify() + " " +
                                                   str(piece.num),
                                                   piec[positions.index(position)]))
                    if(piec[positions.index(position)]=="King 1"):
                        return "Black King Captured! White Wins!"
                    if(piec[positions.index(position)]=="King 2"):
                        return "White King Captured! Black Wins!" 
                    #now need to remove the piece that was captured from the list
                    bp.remove(piec[positions.index(position)])
                    #remove from board as well
                    if(piec[positions.index(position)].identify()+" "+str(
                        piec[positions.index(position)].num) in board[piec[
                            positions.index(position)].position[0]]): 
                        board[piec[positions.index(position)].position[0]].remove(
                        piec[positions.index(position)].identify()+" "+
                        str(piec[positions.index(position)].num))
            piece.move(position)
            #now that the move() method has been invoked, you must adjust the board.
            board[piece.position[0]][piece.position[1]]=piece.identify()+" "+str(piece.num)
            board[previous[0]][previous[1]]="X"
            board=fixBoard(board,bp,wp)
            displayBoard(board) 
    #black turn 
        else:
            print("Black's Turn")
            Notplayable=True
            while(Notplayable):
                piece=bp[random.randint(0,len(bp)-1)] #piece that we're going to move.
                previous=[piece.position[0],piece.position[1]] #previous position of piece
                #check to see if the piece has moved --- need to loop through this.
                if(options[piece.identify()+" "+str(piece.num)]==[]):
                    while(options[piece.identify()+" "+str(piece.num)]==[]):
            ...
            if(canCapture(options,piece)!= False):
                positions=[element[2] for element in canCapture(options,piece)]
                piec2=[element[1] for element in canCapture(options,piece)]
                if(position in positions):
                    board[position[0]][position[1]]=piece.identify() + " "+str(piece.num)
                    print("{} captured {}!".format(piece.identify()+" "+str(piece.num),
                                                   piec2[positions.index(position)])
                          )
                    #checking to see if the king is captured; if so then the game ends..
                    if(piec2[positions.index(position)]=="King 1"):
                        return "Black King Captured! White Wins!"
                    if(piec2[positions.index(position)]=="King 2"):
                        return "White King Captured! Black Wins!"
                    #now remove the captured piece from the lists
                    wp.remove(piec2[positions.index(position)])
            piece.move(position)
            board[position[0]][position[1]]=piece.identify()+" " + str(piece.num)
            board[previous[0]][previous[1]]="X"
            board=fixBoard(board,bp,wp)
        ...
        if(c1==True and c2==True):
            keepGoing=False
        #declaring stalemate, if turn is 100 mo moves
        if(turn==100):
            keepGoing=False
            return("STALEMATE Nobody won!!!")
    if(turn%2==0):
        print("White won game in {} many turns!".format(turn))
    else:
        print("Black won game in {} many turns!".format(turn))
    displayBoard(board) 
